...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (id) {
      ...
      const record = { id: r[0], ext_id: r[1], ts: r[2], created: r[3], answers: JSON.parse(r[4]) };
    ...
    if (ids) {
      ...
      const result = await db.execute({ sql: `SELECT * FROM responses WHERE id IN (${placeholders}) ORDER BY id`, args: idList });
      rows = result.rows;
    } else {
      const result = await db.execute("SELECT * FROM responses ORDER BY id");
      rows = result.rows;
    }
